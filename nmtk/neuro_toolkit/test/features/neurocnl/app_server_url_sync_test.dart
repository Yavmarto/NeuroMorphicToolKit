...
void main() {
  ...
  testWidgets('mounting with initialServerUrl checks the connection instead of '
    ...
    await tester.pumpWidget(
      buildNeurocnlSurfaceTestHost(
        initialServerUrl: 'http://10.0.0.5:9000/api/neurocnl',
      ),
    );
    ...
    expect(
      ...
      'http://10.0.0.5:9000/api/neurocnl',
    );
  });
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(buildApp('http://10.0.0.5:9000/api/neurocnl'));
      await tester.pump();
      await tester.pump();

      // Same widget/State reused (no key change) — this exercises
      // didUpdateWidget, not a remount.
      await tester.pumpWidget(buildApp('http://10.0.0.9:9000/api/neurocnl'));
      await tester.pump();
      await tester.pump();
      ...
      expect(
        container.read(serverConfigProvider).serverUrl,
        'http://10.0.0.9:9000/api/neurocnl',
        reason:
