  ...
  group('formatDeployError', () {
    ...
    test('keeps runtime proxy details for connectivity failures', () {
      final message = formatDeployError(
        const LauncherControlApiException(
          502,
          '{"error":"Runtime request failed for POST http://192.168.2.51:8002/hardware/pynq/deploy: could not be reached: <urlopen error [Errno 61] Connection refused>","path":"/api/launcher/pynq/boards/board-1/deploy","method":"POST"}',
        ),
        serviceName: 'launcher control service',
      ...
      expect(message, contains('Could not reach'));
      expect(message, contains('http://192.168.2.51:8002'));
    ...
    test('extracts nested runtime error bodies from launcher proxy failures', () {
      final message = formatDeployError(
        const LauncherControlApiException(
          500,
          '{"error":"Runtime request failed for POST http://192.168.2.51:8002/hardware/pynq/deploy: HTTP 500","runtimeBody":"{\\"detail\\":{\\"detail\\":\\"Overlay load failed: No Devices Found\\",\\"error_code\\":\\"PYNQ_OVERLAY_LOAD_FAILED\\"}}"}',
        ),
        serviceName: 'launcher control service',
        action: 'deploying to the PYNQ board',
      );
