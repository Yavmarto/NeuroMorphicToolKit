...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        capturedRequest.url.toString(),
        'http://10.0.0.9:8002/api/neurochip/akida/deploy/mapped?bit_width=4',
      );
  ...
  test(
    ...
    () async {
      ...
      expect(
        capturedRequest.url.toString(),
        'http://10.0.0.9:8002/api/neurochip/akida/map?bit_width=4',
      );
      expect(capturedRequest.headers['X-API-Key'], 'token-123');
      expect(capturedRequest.headers['Content-Type'], 'application/json');
      expect(jsonDecode(capturedRequest.body), {'graph': 'ok'});
  ...
  test(
    ...
    () async {
      ...
      expect(
        capturedRequest.url.toString(),
        'http://10.0.0.9:8002/api/neurochip/akida/inference',
      );
      expect(capturedRequest.headers['X-API-Key'], 'token-123');
      expect(capturedRequest.headers['Content-Type'], 'application/json');
      expect(jsonDecode(capturedRequest.body), {
