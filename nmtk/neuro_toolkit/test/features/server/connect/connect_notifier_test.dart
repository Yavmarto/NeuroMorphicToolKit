  ...
  test(
    ...
    () async {
      ...
      await container
          ...
          .connect(const ConnectRequest(host: '203.0.113.90'));

      final state = container.read(connectNotifierProvider);
      expect(state.phase, ConnectPhase.connected);
      expect(state.session?.host, '203.0.113.90');
      expect(state.savedHost, '203.0.113.90');
      expect(service.probeCalls, 1);
      expect(service.lastProbedHost, '203.0.113.90');

      final store = await container.read(targetStoreProvider.future);
      final saved = await store.loadLastTarget();
      expect(saved?.host, '203.0.113.90');
    },
  ...
  test(
    ...
    () async {
      ...
      await store.saveTarget(
        const ConnectTarget(host: '192.0.2.5', appUsername: ''),
      );
      ...
      final state = container.read(connectNotifierProvider);
      expect(state.phase, ConnectPhase.connected);
      expect(state.savedHost, '192.0.2.5');
      expect(service.lastProbedHost, '192.0.2.5');
    },
        ...
        overrides: [
          ...
          targetStoreProvider.overrideWith((ref) async {
            ...
            await prefs.setString(
              'connect.targets.v1',
              '[{"host":"192.0.2.5","appUsername":"","updatedAt":"2026-09-13T00:00:00.000"}]',
            );
            return TargetStore(
              preferences: prefs,
