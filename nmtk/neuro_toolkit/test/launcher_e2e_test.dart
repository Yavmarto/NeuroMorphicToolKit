  ...
  test(
    ...
    () async {
      ...
      try {
        ...
        final venvPath =
            p.join(neurocnl.directory, neurocnl.sourcePath, 'venv');
        expect(Directory(venvPath).existsSync(), isTrue,
            reason: 'venv directory should exist');

        final pythonBin = Platform.isWindows
            ? p.join(venvPath, 'Scripts', 'python.exe')
            : p.join(venvPath, 'bin', 'python');
        expect(File(pythonBin).existsSync(), isTrue,
            reason: 'python binary should exist in venv');
        ...
        expect(lastStatus?.status,
            anyOf(ModuleStatus.starting, ModuleStatus.running));
        ...
        final nsVenvPath =
            p.join(neurosim.directory, neurosim.sourcePath, 'venv');
        expect(Directory(nsVenvPath).existsSync(), isTrue,
            reason: 'Neurosim venv directory should exist');
