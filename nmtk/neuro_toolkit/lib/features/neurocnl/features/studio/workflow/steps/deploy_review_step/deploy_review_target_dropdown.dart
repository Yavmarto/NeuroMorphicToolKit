      ...
      items: [
        for (final id in reviewableTargets)
          DropdownMenuItem<String>(
            ...
            child: Builder(
              builder: (context) {
                final hasResult = ref.watch(deployTargetHasResultProvider(id));
                return Text(
                  hasResult
                      ? targetLabel(id)
        ...
        if (!reviewableTargets.contains(selectedTarget))
          DropdownMenuItem<String>(
            value: selectedTarget,
            child: Text(targetLabel(selectedTarget)),
          ),
      ],
      onChanged: (target) {
        if (target != null) {
          ref.read(workspaceProvider.notifier).setSelectedDeployTarget(target);
        }
      },
    );
  }
