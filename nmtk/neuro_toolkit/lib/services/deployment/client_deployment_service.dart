...
''');
  ...
  Future<DeploymentJob> setupRemoteServer(
    ...
    _runningJobs[jobId] = _runRemoteSetup(job, target, request, bundle);
    unawaited(
      _runningJobs[jobId]!.whenComplete(() {
  ...
  Future<void> _runRemoteSetup(
    ...
    try {
      ...
      final bootstrap = await _bootstrapRemoteUser(
        ...
        onPhase: (phase, percent, label) => _emit(job, phase, percent, label),
  ...
  Future<DeploymentJob> cancelJob(String jobId) async {
    final cancelledBootstrap = _adminSessions.cancel(jobId);
  ...
  Future<void> retryJupyter(String targetId) async {
    ...
    if (target.targetType == 'remote_host') {
      ...
      await _runLocalChecked(request.containerEngine, [
        'compose',
        '--project-name',
        'nmtk',
        '-f',
    ...
    final matchingJobs = _jobs.values
  ...
  Future<SystemHealthReport> diagnoseTarget(String targetId) async {
    ...
    final target = matches.first;
    return diagnoseHost(target.host, backendPort: target.backendPort);
  }
  ...
  Future<SystemHealthReport> diagnoseHost(
    ...
    try {
      final response = await _httpClient
          .post(
            Uri.parse('http://$effectiveHost:$backendPort/api/suite/doctor'),
    ...
    try {
      final response = await _httpClient
          .get(
            Uri.parse(
              'http://$effectiveHost:$_launcherControlPort/api/launcher/doctor',
            ),
          )
  ...
  Future<void> _startRemoteDeployment(
    ...
    try {
      ...
      await _uploadAssets(client, deployDir, job: job, request: request);
  ...
  Future<SSHClient> _connect(DeploymentRequest request) async {
    return _ssh.connect(request: request, persistence: await _store);
  }
...
''',
  ...
  Future<void> _uploadAssets(
    ...
    if (request.adminToken.isNotEmpty) {
      ...
      await _runChecked(
        client,
        'chmod 600 ${shellQuote(path.posix.join(credentialsDir, 'admin-token'))}',
  ...
  Future<DeploymentJob> _verifyRemoteApis(
    ...
    final modulesResponse = await _waitForResponse(
      Uri.parse('http://$host:$_launcherControlPort/api/launcher/modules'),
      attempts: attempts,
    );
    ...
    await _startAndVerifyNeuroStudio(
      host,
      target.backendPort,
      attempts: attempts,
    );
  ...
  _updateSelectedAkidaRuntime(DeploymentJob job, Uri launcherBase) async {
  ...
  Future<SSHRunResult> _runRemoteCommand(
    ...
    void forward(String line) {
      ...
      streamedUpdates = streamedUpdates.then(
        (_) => _appendTerminalOutput(job.id, safeLine),
      );
    }
  ...
  Future<void> _setActiveOperation(
    ...
    _operationHeartbeatTimers[jobId] = Timer.periodic(
      ...
      (_) {
        ...
        unawaited(_updateJob(latest.copyWith(updatedAt: DateTime.now())));
      },
    );
  }
