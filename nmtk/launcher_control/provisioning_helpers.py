    ...
    *,
    ...
    return "\n".join(
        [
            ...
            '    sudo_cmd mkdir -p "$(dirname "$INSTALL_STATUS_PATH")"',
            '    printf "%s\\n" "$STATUS_JSON" | sudo_cmd tee "$INSTALL_STATUS_PATH" >/dev/null',
            '    sudo_cmd chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_STATUS_PATH"',
            '  else',
            ...
            '  printf "INSTALL_STATUS_JSON=%s\\n" "$(sudo_cmd cat "$INSTALL_STATUS_PATH")"',
            "else",
