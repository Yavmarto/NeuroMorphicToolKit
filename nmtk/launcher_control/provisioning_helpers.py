    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "import sys",
            "from pathlib import Path",
            "install_status_path = Path(os.environ['INSTALL_STATUS_PATH'])",
            "install_status_path.parent.mkdir(parents=True, exist_ok=True)",
            "payload = {",
            ...
            "}",
            'install_status_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")',
            "PY",
            "}",
            "",
            ...
            '    printf "%s\n" "$TOKEN_VALUE" | sudo_cmd tee "$TOKEN_PATH" >/dev/null',
            '    sudo_cmd chown "$SERVICE_USER:$SERVICE_USER" "$TOKEN_PATH"',
            '    sudo_cmd chmod 0600 "$TOKEN_PATH"',
            "  fi",
            ...
            '  cat <<EOF | sudo_cmd tee "$ENV_DIR/$RUNTIME_SERVICE_NAME.env" >/dev/null',
            "NEUROCHIP_AUTH_ENABLED=true",
            'NEUROCHIP_API_KEY=${TOKEN_VALUE}',
            "EOF",
            '  cat <<EOF | sudo_cmd tee "$ENV_DIR/$CONTROL_SERVICE_NAME.env" >/dev/null',
            ...
            "EOF",
            '  sudo_cmd chown "$SERVICE_USER:$SERVICE_USER" "$ENV_DIR/$RUNTIME_SERVICE_NAME.env" "$ENV_DIR/$CONTROL_SERVICE_NAME.env"',
            '  sudo_cmd chmod 0640 "$ENV_DIR/$RUNTIME_SERVICE_NAME.env" "$ENV_DIR/$CONTROL_SERVICE_NAME.env"',
