"""Provision-time orchestration: deploy jobs, preflight, and root SSH bootstrap.

Everything here either opens an SSH connection (`bootstrap_remote_user`, and
transitively the SSH-mode executor started by `create_job`) or runs a
one-time deploy job. None of it belongs on the connect path — a client that
is only reconnecting to an already-provisioned server never touches this
module. See `deployment_service.py` for the connect-time half (target CRUD,
readiness) that composes an instance of `DeploymentJobRunner` from here.
"""
...
class DeploymentJobRunner:
    """Runs and tracks deploy jobs, and the one-time root SSH bootstrap."""
    ...
    def __init__(
        ...
        *,
        ...
        # Live reachability probe owned by `DeploymentService` (connect-time,
        # no SSH) -- reused here only to annotate a failed job's error detail.
    ...
    def create_job(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not target_id and isinstance(payload.get("target"), dict):
            target_json = self._store.upsert_target(payload["target"])
    ...
    def _run_job(self, job_id: str) -> None:
        ...
        try:
            ...
            job.failure_details = {
                ...
                "existingConnectionReachable": self._is_ready(),
            }
