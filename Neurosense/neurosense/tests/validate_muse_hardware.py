"""Muse 2/S hardware acceptance flow for NeuroSense EEG rehearsal."""
...
async def run_validation(  # noqa: PLR0912, PLR0915
    *,
    ...
    """Run the Muse acceptance flow with either a mock or real board target."""
    checks: list[ValidationCheck] = []
    artifact_path: str | None = None
    resolved_model = muse_model or "muse2"
    ...
    env_patch = {"NEUROSENSE_MUSE_MODEL": resolved_model}
    try:
        with (
            ...
            target = next((device for device in devices if device.type == "muse"), None)
            if target is None:
                report = ValidationReport(
                    passed=False,
                    artifact_path=None,
                    mock=mock,
                    muse_model=resolved_model,
                    checks=[
                        _check("discover target", False, "Muse target was not exposed by scan.")
                    ],
                )
            ...
            checks.append(
                _check(
                    "discover target",
                    True,
                    f"Found {target.name} with {target.channels} EEG channels.",
                )
            )
            ...
            try:
                ...
                checks.append(
                    _check(
                        "connect",
                        connected.connected,
                        f"Connected to {connected.name} at {connected.sampling_rate_hz} Hz.",
                    )
                )
            ...
            captured = len(data) >= 4 and all(bool(channel) for channel in data[:4])
            checks.append(
                _check(
                    "capture samples",
                    captured,
            ...
            sample_array = np.asarray(data[:4], dtype=np.float64)
...
def _default_evidence_path(recordings_dir: Path, mock: bool) -> Path:
    ...
    return recordings_dir / f"muse_acceptance_{suffix}.json"


def _print_report(report: ValidationReport) -> None:
    print("Muse acceptance report")
...
async def _async_main() -> int:
    ...
    parser.add_argument(
        "--muse-model",
        default="muse2",
        help="Muse model to target: muse2, muses, or bled.",
    )
    parser.add_argument(
        "--recordings-dir",
        type=Path,
        default=Path("/tmp/neurosense-muse-run"),
