...
async def test_devices_endpoint_lists_registered_real_board(
    ...
    monkeypatch.setattr(
        pynq,
        "load_pynq_registry",
        lambda: [{"device_id": "pynq", "address": "203.0.113.103"}],
    )
    ...
    assert real["ip_address"] == "203.0.113.103"


def test_load_pynq_registry_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("NEUROSENSE_PYNQ_DEVICES", "pynq@203.0.113.103, edge2 ")
    ...
    assert pynq.node_address(by_id["pynq"]) == "203.0.113.103"
