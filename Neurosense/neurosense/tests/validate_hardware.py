"""Cyton-first hardware acceptance flow for the flagship NeuroSense workflow."""
...
async def run_validation(  # noqa: PLR0912, PLR0915
    *,
    ...
    """Run the Cyton acceptance flow with either a mock or real board target."""
    checks: list[ValidationCheck] = []
    ...
    try:
        with (
            ...
            target = next((device for device in devices if device.type == "cyton"), None)
            if target is None:
                report = ValidationReport(
                    ...
                    checks=[
                        _check("discover target", False, "Cyton target was not exposed by scan.")
                    ],
                )
            ...
            checks.append(
                _check(
                    "discover target",
                    True,
                    f"Found {target.name} at {target.serial_port or serial_port or 'unconfigured port'}.",
                )
            )
            ...
            await recording_service.start(
                ...
                hardware_provenance={
                    ...
                    "serial_port": serial_port or target.serial_port,
                },
            )
...
def _default_evidence_path(recordings_dir: Path, mock: bool) -> Path:
    ...
    return recordings_dir / f"cyton_acceptance_{suffix}.json"


def _print_report(report: ValidationReport) -> None:
    print("Cyton acceptance report")
...
async def _async_main() -> int:
    ...
    parser.add_argument("--serial-port", help="Explicit Cyton serial-port override.")
    parser.add_argument(
        "--recordings-dir",
        type=Path,
        default=Path("/tmp/neurosense-cyton-run"),
