...
def test_hub_login_with_token_writes_config(tmp_path: Path) -> None:
    ...
    with patch("neurocli.hub._CONFIG_PATH", config_path):
        result = runner.invoke(
            app, ["hub", "login", "--registry", "http://example.com", "--token", "tok123"]
        )
...
def test_hub_push_missing_file_exits_1(tmp_path: Path) -> None:
    result = runner.invoke(
        app,
        ["hub", "push", str(tmp_path / "nope.nir"), "-t", "snn_model", "-s", "lif", "-v", "1.0.0",
         "--json"],
    )
    assert result.exit_code == 1
...
def test_hub_push_success_prints_uri(tmp_path: Path) -> None:
    ...
    with (
        ...
        result = runner.invoke(
            app,
            ["hub", "push", str(fake_file), "-r", "http://x.test", "-t", "snn_model",
             "-s", "lif", "-v", "1.0.0", "--tag", "a", "--tag", "b", "--json"],
        )
    assert result.exit_code == 0, result.output
...
def test_hub_pull_success_verifies_checksum(tmp_path: Path) -> None:
    ...
    with patch("neurocli.hub._CONFIG_PATH", tmp_path / "hub.json"), patch(
        "neurocli.hub.httpx.get", side_effect=[meta, blob_resp]
    ):
        result = runner.invoke(
            app,
            ["hub", "pull", "neurohub://snn_model/yoshi/lif@1.0.0", "-r", "http://x.test",
             "-o", str(out), "--json"],
        )
    assert result.exit_code == 0, result.output
...
def test_hub_search_json_shape() -> None:
    resp = _make_response(
        {"items": [{"slug": "lif", "type": "snn_model", "owner": "y", "version": "1.0.0"}],
