...
``neurohub://`` URI grammar. It is copied verbatim into the ``neurocli`` package
(``neurocli/neurocli/uri_parser.py``); the two copies must stay byte-for-byte
equivalent so the CLI and backend agree on parsing (correctness Property 14).
