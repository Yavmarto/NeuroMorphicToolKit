...
make rig-verify RIG=dev            # report one rig
make rig-provision RIG=dev         # sync + bring the rig up, then verify
python3 scripts/fleet_health.py --list-rigs
python3 scripts/fleet_health.py --print-target dev
```
...
| --- | --- | --- | --- |
| `dev` | hardware-rig | `dev@<dev-host>` | dev backend, CI-runner host, Akida, Prophesee, PYNQ |
| `hp-prodesk` | hardware-rig | `<dev-host>` | secondary test rig (profile not recorded yet) |
| `runner-win-wsl2` | ci-runner | `<dev-host>` | Windows + WSL2 self-hosted CI runner |
...
3. Confirm the runtime: `systemctl is-active neurochip.service` must be
   `active`, and `curl http://<dev-host>:8002/health` must answer.
4. The containerized `neurochip-hw-worker` must stay stopped on this rig.

Never run `sudo fuser -k 8002/tcp` on `dev`: that kills the service the
box exists to provide.

### Prophesee EVK (USB3) — `dev`

1. Plug the EVK into a USB3 port.
2. Confirm the bus sees it: `lsusb | grep 1d43`.
3. Apply the overlay and verify:
   `make rig-provision RIG=dev ARGS='--prophesee'`.
4. Run the acceptance script:
   `PYTHONPATH=neurosense python3 -m neurosense.tests.validate_prophesee --api-base http://<dev-host>:8004`.
...
optional missing item, which is why `dev` can still be `OK` without it.
