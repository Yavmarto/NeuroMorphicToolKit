...
1. Create a markdown report. **File Naming Convention**: `docs/archive/{AUDIT_DATE}-status-{AGENT_NAME}.md` (e.g. `docs/archive/29-Mar-2026-status-Jules.md`).
...
- *Date and Agent bindings* (`{AUDIT_DATE}` and `{AGENT_NAME}`) must be injected into your prompt so that the final report file is named correctly.
