...
## Decision
Use `scripts/release.sh` to orchestrate the release process, which calls `scripts/bump_all.py` to synchronize version numbers across all `pubspec.yaml` and `pyproject.toml` files in the monorepo. Docker images are tagged with both the semantic version and the git SHA short hash via `release-docker.yml`. The `scripts/git/push-all.sh` script commits and pushes version bumps atomically across all submodule repositories.
