...
**Classification: PARTIAL — Wave A items are mixed; A1 and A5 DONE, A3 DONE, A2 NOT STARTED, A4 NOT STARTED.**

Checked each Wave A quick win against the actual code:

- **A1 (`MetricDiff.pValue` derived getter) — DONE.**
  `Neurobench/frontend/lib/models/result.dart:93` has exactly the specified
  getter: `double? get pValue => pValueTtest ?? pValueWilcoxon;`, alongside
  the existing `pValueTtest`/`pValueWilcoxon` fields (lines 85-86). Note a
  stale `Neurobench/frontend/build_error.txt` (dated 30 June, pre-fix) still
  shows a compile error for a missing `pValue` getter and a missing
  `runBenchmark` method — that file is a leftover build log from before A1
  landed and should probably be deleted, not evidence of a current failure
  for A1.
- **A2 (`runBenchmark()` in Neurobench `ApiClient`) — NOT STARTED.**
  `grep -rn "runBenchmark" Neurobench/frontend/lib/` returns zero matches in
  live source. `Neurobench/frontend/lib/services/api_client.dart` has no
  `runBenchmark` method (existing methods are `getBenchmarks`,
  `getBenchmark`, `submitBenchmarkRun`, `getBenchmarkJob`,
  `getBenchmarkRunResult`, etc. — no `runBenchmark`). The stale
  `build_error.txt` shows `lib/screens/benchmark_screen.dart:31` used to call
  `apiClient.runBenchmark(...)`, but the current `benchmark_screen.dart` no
  longer contains any `runBenchmark`/`apiClient.` reference at all — the
  call site appears to have been removed rather than the method added,
  meaning this specific caller-shape check point from the plan no longer
  applies as written and A2 needs re-scoping, not just implementation.
- **A3 (dead legacy config fields in `suite_api/config.py`) — verified
  candidate for removal, currently still present (not yet removed), zero
  live readers confirmed.** `suite_api/config.py:16-21` still has all six
  fields (`neurocnl_url`, `neurosim_url`, `neurochip_url`,
  `neurobench_url`, `neurosense_url`, `neurohub_url`). Running the exact
  grep the plan specifies —
  `grep -rn "settings\.neurocnl_url\|settings\.neurosim_url\|settings\.neurochip_url\|settings\.neurobench_url\|settings\.neurosense_url\|settings\.neurohub_url" suite_api/ tests/ scripts/`
  — returns 0 hits, confirming the plan's precondition (safe to delete) but
  the deletion itself has not been done.
- **A4 (restore ADR 0001) — NOT STARTED, and the plan's exact filename is
  now unavailable.** `docs/ADR-claude/0001-module-manifest-system.md` does
  not exist. `docs/ADR-claude/0001-cors-origin-management.md` now occupies
  the `0001` slot (numbers 0002-0020 are all taken by other, unrelated
  ADRs). `nmtk/AGENTS.md:10` still references the broken path
  `docs/ADR-claude/0001-module-manifest-system.md`. This needs a renumbered
  filename (e.g. `0021-module-manifest-system.md`) rather than literally
  `0001`, plus an update to the `nmtk/AGENTS.md` reference.
- **A5 (Linux file picker test) — DONE.**
  `neurocnl/frontend/test/services/file_picker_native_file_backend_test.dart:179-182`
  contains exactly the specified test
  (`'FilePickerDialogGateway installs Linux desktop implementation'`,
  asserting `FilePickerPlatform.instance is FilePickerLinux`).

Waves B, C, D and the "Deployment Open Questions" section were not
individually re-verified in this pass (out of scope for this audit request,
which asked specifically about Wave A) — their status in the doc should be
treated as unconfirmed until checked separately.

**What's still missing:** A2 (method doesn't exist; call site was removed,
so scope needs re-confirming against current `benchmark_screen.dart`), A3
(fields still present, just confirmed safe to delete), and A4 (needs a
non-`0001` filename since that slot is taken, plus the `nmtk/AGENTS.md` fix).
