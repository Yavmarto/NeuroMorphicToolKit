...
**Status: PARTIAL.** Real, git-committed work landed on several phases, but not by literally following this plan — some target files were superseded (e.g. Neurohub's `auth_service.dart` was deleted in favor of a Supabase migration) and several phases show no evidence of execution at all.

Per-phase findings (code checked directly on disk, including inside the `Neurobench`/`Neurohub` submodules):

- **Phase 0 (Triage/Baselines):** NOT STARTED — no `docs/audits/frontend-baseline-*.txt` found; `Neurobench/frontend/build_error.txt` and `Neurohub/frontend/build_error.txt` still contain the original failure traces.
- **Phase 1 (Web builds):** PARTIAL — `MetricDiff.pValue` derived getter exists in `Neurobench/frontend/lib/models/result.dart:93`, but `runBenchmark` was never added to `Neurobench/frontend/lib/services/api_client.dart` (still missing), so `benchmark_screen.dart:31` is still broken. Neurohub added `shared_preferences: ^2.5.5` to pubspec, but `auth_service.dart` was deleted entirely in a later Supabase-auth migration, and `Dockerfile:14` still lacks `--no-wasm-dry-run`.
- **Phase 1C (integration test imports):** NOT STARTED — `Neurobench/integration_test/e2e_test.dart:3` still imports `package:frontend/app.dart`; `Neurohub/frontend/integration_test/app_test.dart:8-9` still imports dead `project_card.dart`/`activity_feed.dart`.
- **Phase 2 (Neurosense data classes):** PARTIAL — `device_provider.dart`, `recording_provider.dart`, `sessions_provider.dart`, `quality_provider.dart` and their model files were migrated to `@freezed`/`@riverpod`, but `stream_provider.dart` and `preset_provider.dart` were not. `live_signal_viewer.dart` still has the `expandedHeight` param (2B not done). `ProviderScope` nesting fix (2C) appears resolved — override now lives in `main.dart`.
- **Phase 3 (nmtk_ui_core hardening):** NOT STARTED — no `nmtk_ui_core/lib/src/zeta_fallback.dart`; `desktop_scaffold.dart:1` still suppresses `lines_longer_than_80_chars`; `lib/neat/dark/` still ships under `lib/`, not moved to `previews/`; `NmtkToasts` still exists, not marked `@Deprecated`.
- **Phase 4 (Zeta sweep):** NOT STARTED — `grep -rn "ZETA-MIGRATION-TODO"` still returns ~15 hits across Neurobench, Neurohub, and Neurosense (target was 0).
- **Phase 5 (NDH shell/core):** DONE — `neurodreamhand/shell/adapter.py` and `hitl_surface.py` use narrowed exception tuples; `neurodreamhand/core/_constants.py` exists with the `ERROR_SCALE=1.0` vs OCL-override `5.0` split called out in the plan, consumed by `runner.py`, `drop_test.py`, and `ocl_engine.py`.
- **Phase 6 (Neurohub stubs):** MOSTLY DONE — all 7 stub widgets (`config_panel.dart`, `workflow_step_card.dart`, `member_manager.dart`, `note_editor.dart`, `bundle_export_dialog.dart`, `milestone_timeline.dart`, `live_test_dashboard.dart`) now render `NmtkEmptyState`. `_launchModule` in `project_detail_screen.dart:174-179` still has `launchUrl` commented out (6C not done).
- **Phase 7 (Contract sync):** NOT STARTED — no `nmtk_module_contracts_gen/` directory and no `scripts/sync_dart_contracts.py`.
- **Phase 8 (Cross-frontend polish):** NOT STARTED (spot-checked) — no `_getList` helper in Neurohub's `api_service.dart`; dead imports in `integration_test/app_test.dart` (L9) still present.
- **Phase 9 (Verification sweep):** NOT STARTED — stale `build_error.txt` artifacts indicate `flutter build web` still fails for both Neurobench and Neurohub.
- **Phase 10 (Open Brain):** NOT STARTED — no evidence these lessons were logged.

**What's missing overall:** the two flagship acceptance criteria (clean `flutter build web` for Neurobench/Neurohub, and zero `ZETA-MIGRATION-TODO` markers) are both unmet, and Phases 0, 1C, 3, 4, 7, 8, 9, 10 show no execution. The plan's own closing line ("No files have been modified... Awaiting approval") is stale — partial work happened outside the sequencing this doc describes.
