...
**Classification: DONE (methodology doc) / PARTIAL (underlying codegen fixes it documents)**

This file is primarily a reusable *method* doc, not itself a work item — but it
opens by asserting a specific prior fix is "done" (`paper/02_cnn/gen_snn.ipynb`,
9.8% -> ~98%). That underlying claim was checked against the actual codebase:

- Confirmed present in `neurocnl/backend/app/routers/notebook.py`: the three
  hardcoded-constant fixes the doc describes for notebook 2 all exist with
  explanatory comments citing the oracle, e.g. `reset_delay=False` (lines
  476, 507, 619-625, 636-639), `divisor_override=1` for `SumPool2d` (lines
  593-599), and the `nir.IF` -> `snn.Leaky(beta=1)` mapping (git commit
  `d0a25e65 fix: map nir.IF to snn.Leaky(beta=1) instead of Lapicque`, in the
  `neurocnl` submodule history).
- Git log for `neurocnl/backend/app/routers/notebook.py` shows a matching
  sequence of iterative fix commits (`d17fe76c`, `3fa137bd`, `5ddd826d`,
  `65edcf87`, `d0a25e65`, `75aa65fd`, `a5ecd79b`, `ab8526b8`, `799e3da4`,
  `dbb0944b`), consistent with the doc's "three independent mismatches, fixed
  one at a time" narrative.
- Regression tests exist: `neurocnl/backend/tests/test_notebook_generate_v2.py`
  and `test_notebook_codegen.py` (both present, recently modified July 3-4).
  Could not execute them in this environment — `pytest` collection fails with
  an unrelated, pre-existing `matplotlib`/`nengo` import error
  (`ImportError: cannot import name '_c_internal_utils'`) in the local
  miniconda env, not a code defect — so pass/fail state wasn't directly
  re-verified here.
- `paper/02_cnn/gen_snn.ipynb` itself no longer exists on disk (paper/ is
  gitignored, and the doc explicitly treats these as disposable verification
  artifacts, not something to keep committed) — its prior 9.8%->98% result
  could not be re-confirmed by re-running, only inferred from the code fixes
  still being in place.

**What's still missing:** the doc's stated purpose — applying this method to
the other 4 notebooks (`01_lif/lif_snntorch.ipynb`,
`03_rnn/Braille_training_snntorch.ipynb`,
`03_rnn/snntorch_apply_subtract.ipynb`, `03_rnn/nengo_apply.ipynb`) — has no
direct evidence of completion or non-completion in this pass; the doc itself
is written as forward-looking work still to do, and nothing in `notebook.py`
or `paper/03_rnn`/`paper/01_lif` was checked node-type-by-node-type against
this playbook's specific per-notebook notes (RSynaptic/Synaptic reset
mechanism, Nengo codegen path) to confirm those 4 are now fixed.
