...
**Status: DONE** — all 5 tasks landed in commit `62566f7` ("feat: NMTK June 30 — canvas↔training fix,
deployment_mode, learning rule canvas projection", 2026-06-30) plus submodule bump `9af1842`.

- **Task 1 (canvas ↔ training disconnect):** DONE. `neurocnl/neurocnl/training/snntorch_adapter.py`
  `SnnTorchAdapter.run()` now passes `request.nir_graph` through to a private builder that deserializes the
  NIR graph and only falls back to the hardcoded `TinySnn` (with a logged warning,
  "snntorch_adapter: no nir_graph provided, using TinySnn fallback") when the graph is absent/unparseable.
  `neurocnl/backend/app/services/training_service.py` has `_compile_cnl_to_nir_graph()` and wires it into
  `submit_training_job()` (`nir_graph = _compile_cnl_to_nir_graph(cnl_spec)`).
- **Task 2 (`deployment_mode` on `LearningRuleIR`):** DONE. `neurocnl/neurocnl/ir/types.py:156-166` adds
  `deployment_mode: Literal["offline_train", "online_learn", "deploy_only"]`. `nir_exporter.py` derives
  `ONLINE_LEARN_BACKENDS` from `capabilities.py` concept-support verdicts (not hardcoded) and both
  `validate_deployment_mode_fidelity()` and `audit_deployment_mode()` raise `CompileError` /
  `unsupported_deployment_mode` errors when `online_learn` targets a non-Lava/Loihi backend.
- **Task 3 (`hasLearningRule` wired into canvas):** DONE. `canonical_editor_document.dart`'s `CanvasEdge`
  has `hasLearningRule`/`learningRuleKind` fields (with JSON (de)serialization); `canvas_projection_utils.dart`
  copies them into the `CanvasGraph` edge `parameters` map; `network_canvas.dart:1899-1901` renders a dashed
  stroke ("8 px on / 5 px off") when `parameters['hasLearningRule'] == true`.
- **Task 4 (ruff sweep):** PARTIAL. No commit matching "chore: ruff auto-fix 219 violations" was found;
  the nearest ruff-fix commits (`2a52a85`, `a674b93`) predate this plan (April 2026). A fresh
  `ruff check neurocnl/ suite_api/ Neurohub/ Neurosim/ Neurosense/ Neurochip/ Neurobench/` run today reports
  170 errors (34 auto-fixable), so error count dropped substantially since the plan's "557 errors" baseline
  but it's unclear whether that came from this specific sweep or incidental fixes in other commits.
- **Task 5 (`scratch.py` deletion):** DONE. `neurocnl/backend/app/scratch.py` does not exist; commit
  `62566f7`'s message explicitly lists "Removed debug scratch.py from backend package."

**Missing:** confirmation of a dedicated ruff auto-fix commit as specified (Task 4 may have been folded into
other work rather than run as an isolated, separately-committed sweep).
