...
**Status: PARTIAL — built, but shipped disabled behind a default-off flag due to an unfixed crash.**

Evidence the work was actually done (this is real, substantial code, not just planning):
- Rust crate exists at `neurocnl/frontend/rust/nmtk_wgpu/` (`Cargo.toml`, `src/lib.rs` 241 lines, `src/renderer.rs` 518 lines, `src/spike_buffer.rs`, `src/density_buffer.rs`, `src/texture_bridge/{mod.rs,macos.rs}`), matching the plan's proposed `nmtk_wgpu` crate.
- A second Rust crate/plugin, `rust/nmtk_wgpu_renderer_plugin/`, wraps it as a Flutter macOS plugin (Podspec, Swift `NmtkWgpuRendererPlugin`, `lib/nmtk_wgpu_renderer_plugin.dart`).
- Flutter side: `nmtk_ui_core/lib/visualization/wgpu_native_renderer.dart` (168 lines) implements `WgpuNativeNeuronRenderer` against the `NeuronRenderer` interface (`nmtk_ui_core/lib/visualization/renderer_interface.dart`), plus FFI bindings in `nmtk_ui_core/lib/visualization/src/nmtk_wgpu_bindings.dart` (106 lines).
- `nmtk_ui_core/lib/visualization/renderer_registry.dart` implements exactly the "seamless swap" from the plan (`createNeuronRenderer()` picks between `WgpuNativeNeuronRenderer` and `FragmentShaderNeuronRenderer`).
- Wired into the app: `neurocnl/frontend/lib/screens/viz_demo_screen.dart`, `viz_demo_provider.dart`, and `app_router.dart` reference `/viz-demo`. Git history shows real iteration: `cf596b4c feat: native wgpu neuron renderer for viz-demo (10k/100k particles, 1M density)` plus follow-on commits for chip-die tile targets/renderer.

What's missing / diverges from the plan:
- **The native renderer is hard-disabled by default.** `renderer_registry.dart` gates it behind `kEnableWgpuRenderer = bool.fromEnvironment('NMTK_WGPU_RENDERER', defaultValue: false)`, with this comment in the code: *"`SharedPixelBuffer::write_rows` (rust/nmtk_wgpu/src/texture_bridge/macos.rs) crashes the whole process with SIGSEGV on the first real draw call (confirmed via macOS crash report — objc_msgSend_uncached failure inside CVPixelBufferLockBaseAddress). Re-enable via `--dart-define=NMTK_WGPU_RENDERER=true` only for native-side debugging."* So in production builds the app always falls back to the Phase 2 `FragmentShaderNeuronRenderer`, i.e. the 1M-neuron goal this plan targets is not actually reachable by end users today.
- Even with the flag on, only macOS/Metal is gated in (`Platform.isMacOS` check) — no Android/Vulkan or Windows/DirectX12 path is wired into `renderer_registry.dart`, despite the plan describing all three platforms.
- No evidence of the "Texture Registration" verification step (solid-color texture sanity check) or a documented 60 FPS demo run — the crash comment suggests the team got as far as "first real draw call" and stopped there.
- Verification Plan item 3 ("Demo Execution... maintain a stable 60 FPS using the Rust backend") is not verifiable as done since the backend is disabled by default.
