...
**Status: PARTIAL — Phase 0 done, parts of Phase 2/3 landed (differently than sketched), Phase 1 (bundle container) and Phase 4/4b (BindsNET/Lava online adapters) NOT STARTED.**

Done, and further than this doc's "needs adding" table (Section 6) suggests:
- `deployment_mode: Literal["offline_train", "online_learn", "deploy_only"]` now exists directly on `LearningRuleIR` (`neurocnl/neurocnl/ir/types.py:166-168`) — the doc's table marked this "❌ Not in `LearningRuleIR` or `TrainingRequest`"; it landed on `LearningRuleIR` instead of `TrainingRequest`.
- The Lava-only online-learning constraint (Problem 3 / Phase 3 item 3-4) is enforced: `validate_deployment_mode_fidelity` and `audit_deployment_mode` in `neurocnl/neurocnl/export/nir_exporter.py:80-115,344-370` raise `deployment_mode_unsupported`/`unsupported_deployment_mode` errors when `online_learn` is requested against a non-Lava backend.
- Phase 0 ("fix the adapter–canvas disconnect") item 1 is DONE: `neurocnl/neurocnl/training/snntorch_adapter.py:71-251` — `_run_real()` now accepts `nir_graph` from the request payload and only falls back to the hardcoded `TinySnn` topology when no graph is supplied (log line: `"snntorch_adapter: no nir_graph provided, using TinySnn fallback"`).
- Phase 2 items 1 and 4 (learning-rule field on canvas edges + dashed/plastic edge rendering) are DONE: `neurocnl/frontend/lib/models/canonical_editor_document.dart:125,145,155,167` adds a `learningRuleKind` field (simpler than the doc's proposed full `LearningRuleProjection` object, but functionally equivalent for kind display), and `neurocnl/frontend/lib/widgets/canvas/network_canvas.dart:1899-1970` renders the badge/dashed styling for edges with `hasLearningRule`. Confirmed by neurocnl submodule commit "feat: wire hasLearningRule into canvas projection + dashed edge painter" (2026-06-30).

Still missing (doc's Section 6 table is still accurate here):
- No `ProjectBundle` class anywhere in the codebase (`grep -rn "class ProjectBundle" neurocnl/neurocnl` — zero hits).
- No `NetworkIR.to_dict()`/`from_dict()` JSON serializer (`grep -rn "def to_dict|def from_dict" neurocnl/neurocnl --include="*.py"` — zero hits) — Phase 1 not started.
- No `.nmtk` ZIP bundle read/write code and no bundle `manifest.json` writer (the only `manifest.json`/`zipfile` hits in the codebase belong to unrelated PYNQ deployment-artifact contracts, not this bundle format).
- No `reward_signal` field on `LearningRuleIR` (`grep -rn "reward_signal" neurocnl/neurocnl` — zero hits) — Problem 5 fix not applied.
- No BindsNET adapter anywhere (`find neurocnl -iname "*bindsnet*"` — zero hits) — Phase 4 not started.
- No `LavaOnlineAdapter` class (`grep -rn "LavaOnlineAdapter" neurocnl/neurocnl` — zero hits); only the audit/validation functions reference `online_learn`, there is no actual Lava online-learning adapter yet — Phase 4b not started.
- Phase 2 item 5 (edge parameter inspector dropdown to pick a learning rule kind) is NOT done: `neurocnl/frontend/lib/widgets/canvas/property_panel.dart` has zero references to `learningRule`.

Net effect: the structural/training-backend halves described as "✅ complete" or "✅ scaffolded" in Section 8 remain accurate, and Phase 0 has now shipped (it was open when this doc was written). The bundle container (Phase 1) and both online-learning adapters (Phase 4/4b) remain the real open work.
