...
**Canvas Drawing Model — PARTIAL (more done than this doc shows; doc is stale by ~3 weeks of submodule commits).**
- Component 3 (edge learning rule badges): HALF DONE. `neurocnl/frontend/lib/widgets/canvas/network_canvas.dart:1899-1970` renders the `hasLearningRule`/`learningRuleKind` badge on canvas edges (confirmed by submodule commit `feat: wire hasLearningRule into canvas projection + dashed edge painter`, 2026-06-30). But `neurocnl/frontend/lib/widgets/canvas/property_panel.dart` has zero references to `learningRule`/badge — the property-panel learning-rule section is still NOT implemented.
- Component 4 (export_screen.dart new formats): DONE, contradicting the "screen not updated" note. `neurocnl/frontend/lib/screens/canvas/export_screen.dart:10,124` imports and calls `NotebookGenerateService.generate(ref, context)`.
- Component 6 (Dart tests): DONE. `neurocnl/frontend/test/models/pipeline_config_test.dart` exists; `flutter test` on it passes 11/11.
- Python tests: file exists at `neurocnl/backend/tests/test_notebook_generate_v2.py`, but could not be executed here — local pytest collection fails on an unrelated environment error (`ImportError: cannot import name '_c_internal_utils' from ... matplotlib`), not a code defect.
- Remaining real gap: property_panel.dart learning-rule UI only.

**Material Icons → Zeta Icons Migration — IN PROGRESS, doc's numbers stale.**
- `grep -ro "Icons\.\w" neurocnl/frontend nmtk Neurohub Neurosense Neurosim Neurochip Neuro-Dream-Hand` returns 660 hits (doc cites 631 across 112 files in 6 packages) — still substantial live usage.
- 267 `ZETA-MIGRATION-EXEMPT` markers already exist in `.dart` files, showing Tier C1 exemption work is well underway (more progress than the unchecked boxes suggest).
- `.kiro/specs/material-to-zeta-icons-sweep/` and `.kiro/specs/material-to-zeta-button-sweep/` do **not** exist in the working tree — T-ICON-1 (Kiro spec governance tests) is confirmed NOT STARTED.
- No checkbox items in this doc can be marked done; sweep tasks T-ICON-2/3 evidence not distinguishable from ongoing background icon replacement commits.
