...
*   **LIF Nodes:** The `snn.Leaky` neuron uses a `beta` (decay rate) of `0.99` and a default `threshold` of `1.0`. CNLStudio parameterizes LIF using biological constants. Assuming a discrete time-step $\Delta t = 1$, the relation is roughly $\beta \approx 1 - \frac{\Delta t}{\tau}$. Thus, $\tau = \frac{1}{1 - \beta} = \frac{1}{0.01} = 100$.
    *   `tau` = 100
    *   `threshold` = 1.0
    *   `r` = 1 (default)
    *   `v_leak` = 0 (default)
...
3.  **LIF Node:** Set `n_neurons = 1000`, `tau = 100`, `threshold = 1.0`, `r = 1`, `v_leak = 0`
4.  **Linear Node:** Set `rows = 1000`, `cols = 10`, `weight_fill = 0.05`
5.  **LIF Node:** Set `n_neurons = 10`, `tau = 100`, `threshold = 1.0`, `r = 1`, `v_leak = 0`
...
### 3. Canvas: Train (Exporting to NIR)
...
Navigate to the **Results** step and open the **Dynamics** tab.
