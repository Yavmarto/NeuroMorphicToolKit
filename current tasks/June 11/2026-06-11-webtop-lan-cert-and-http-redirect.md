...
**Status: DONE.** Every file this plan names exists at the repo root with the described content, and this matches `MASTER_TASK_ORDER.md`'s own "Previously Completed" table entry for this item.

- `Dockerfile.webtop` (repo root) — downloads the mkcert static binary (`MKCERT_VERSION`, `curl ... mkcert-v${MKCERT_VERSION}-linux-amd64`) and `COPY scripts/webtop-mkcert-startup.sh /custom-services.d/00-mkcert-startup.sh`, matching the plan's "Download mkcert static binary; COPY mkcert init script" action.
- `scripts/webtop-mkcert-startup.sh` exists and implements the idempotency + fix described in this doc's own "later same day" postmortem section: SAN dedup via `awk 'NF && !seen[$0]++'`, an awk-based splice (line ~180: "replace the *first* `server { ... }` block whose `listen` line is `3000 default_server`") that leaves the SSL block intact, log lines `"issuing cert for: ..."` and `"cert already valid for current SANs; skipping"`, and it ends with `exec sleep infinity` (line 304) — exactly the fix this doc describes for the s6-longrun respawn-loop bug.
- `scripts/webtop-http-redirect.conf` and `scripts/webtop-trust-ca.sh` both exist at the described paths.
- `docker-compose.webtop.yml` (repo root) has `ports: "${WEBTOP_PORT:-3030}:3000"` and `"${WEBTOP_HTTPS_PORT:-3031}:3001"` plus `WEBTOP_HTTPS_PORT` env — matches the plan's port-mapping spec exactly.
- `Makefile` has `WEBTOP_HTTPS_PORT ?= 3031`, a `webtop-trust` target calling `scripts/webtop-trust-ca.sh`, and `webtop-up` invoking `webtop-trust` automatically (`$(MAKE) --no-print-directory webtop-trust || true`), matching "auto-calls `webtop-trust` on first run."
- `docs/webtop-browser-desktop.md` exists and documents the two-port mapping and `webtop-trust` step.

Nothing outstanding was found for this plan — no further action needed. (Not independently re-run: the live `curl`/`openssl` verification steps in this doc's "Verification" section, since that requires actually building and starting the `webtop` container, which was out of scope for this static-code audit.)
