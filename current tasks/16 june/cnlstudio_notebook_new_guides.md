...
7. Add a **Data Loader** node: `format=pt` (or `npy`), `dataset_path=<path to the saved d tensor>`, `batch_size=1`, `shuffle=false`.
8. Add a **State Reset** node.
...
10. Connect the node ports as follows:
    ...
    *(Note: remaining output ports such as `membrane` and `model` on the Forward Pass node should be left unconnected — there's no Accuracy node here since this notebook has no labels, only a raw spike/voltage trace.)*
11. Click **Run Eval**.

**Canvas: Train** (Export)

12. Add a **NIR Exporter** node: `filename=lif_norse.nir`.
13. Connect the trained/built model's `model` output port into the NIR Exporter's `model` input port.
...
2. Same Data Loader / State Reset / Forward Pass steps as `lif_norse.ipynb`'s guide, steps 7–11 (same stimulus workaround applies — Nengo simulation timing in the notebook uses `dt=1e-4`, `presentation_time=dt`, matching the same spike train).
