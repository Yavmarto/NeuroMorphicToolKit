...
**Status: PARTIAL — Tier A/B sweep substantially done, Tier C1 markers in place, T-ICON-1 spec artifacts missing, doc numbers stale.**

Evidence (live grep across the six packages' `lib/`, 2026-07-04):

| Package | `Icons.*` hits (was) | `ZetaIcons.*` hits | `ZETA-MIGRATION-EXEMPT` markers |
|---|---:|---:|---:|
| `nmtk_ui_core` | 16 (was 59) | 48 | 14 |
| `neurocnl/frontend` | 73 (was 316) | 295 | 135 |
| `Neurohub/frontend` | 15 (was 64) | 50 | 17 |
| `Neurobench/frontend` | 5 (was 48) | 32 | 19 |
| `Neurosense/frontend` | 21 (was 60) | 40 | 21 |
| `nmtk/neuro_toolkit` | 25 (was 84) | 51 | 31 |
| **Total** | **155 (was 631)** | **516** | **237** |

- **T-ICON-1 (spec PR):** NOT STARTED. No `.kiro/specs/material-to-zeta-icons-sweep/` directory exists anywhere in the repo (checked via `find`), so the curated Tier B mapping CSV and design doc referenced throughout §3/§4 and the governance test comments were never landed.
- **T-ICON-2 (governance tests):** DONE. `test/governance/material_icons_audit_test.dart` exists in all six packages (`nmtk_ui_core`, `neurocnl/frontend`, `Neurohub/frontend`, `Neurobench/frontend`, `Neurosense/frontend`, `nmtk/neuro_toolkit`) implementing the exact `iconsHits - markerCount <= kIconsBaseline` ratchet described in §4.1. Ran all six via `flutter test test/governance/material_icons_audit_test.dart` — **all pass** (each package's `kIconsBaseline` has been ratcheted down from its initial baseline; e.g. `nmtk_ui_core`'s is `0`).
- **T-ICON-3/4 (Tier A mechanical + Tier B curated sweep):** DONE, in substance. `Icons.*` usage dropped 631→155 (75% reduction) and `ZetaIcons.*` usage rose from 0→516, consistent with Tier A/B substitutions actually landed in code (not just planned).
- **T-ICON-5 (Tier C1 exemption markers):** DONE. 237 `ZETA-MIGRATION-EXEMPT:` markers exist across the six packages, comfortably covering the 155 remaining `Icons.*` hits (governance tests confirm `unannotated <= baseline` per package).
- **T-ICON-6 (final verification / zero unannotated hits / MASTER_TASK_ORDER update):** NOT STARTED — `MASTER_TASK_ORDER.md` no longer exists in the repo (deleted per `git status`), and no cross-package zero-hit grep has been recorded.

**What's still missing:** the T-ICON-1 spec artifacts (`.kiro/specs/material-to-zeta-icons-sweep/{requirements,design,tasks}.md`) referenced by every governance test's doc comment do not exist — this is a paper-trail gap, not a functional blocker, since the governance tests encode the same baseline logic inline. The doc's §2 inventory table (631 hits / 112 files) and §7 "Execution status" are stale narrative — actual current state is 155 hits, and the sweep is much further along than the doc suggests. No evidence of `nmtk_ui_core/test/models/deployment_icon_resolution_test.dart` (§4.4) — not found via search, so the model-regression gate for `IconData` public API consumers is unverified.
