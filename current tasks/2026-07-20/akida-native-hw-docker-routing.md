...
- End-to-end deploy with `AKIDA_NATIVE=1` against moosebuntu@192.168.2.51.
- `suite_api`'s proxied `/api/neurochip/akida/status` returning real (non-401, non-503) status through the full stack.
- Passwordless `sudo` availability on moosebuntu for `systemctl start neurochip.service` and `cat /opt/neurochip-akida-host/credentials/api-token` — assumed present per existing provisioning-script convention, not re-confirmed here.
