...
The dev backend runs on `<dev-host>`, not locally — a local edit to any Python file under
...
Defaults to `dev@<dev-host>`; override with `REMOTE_HOST=`. Flags go through
...
```bash
ssh dev@<dev-host> 'ls -A ~/nmtk-deploy'
```
...
`<dev-host>` runs a real BrainChip Akida card served by the native `neurochip.service`
