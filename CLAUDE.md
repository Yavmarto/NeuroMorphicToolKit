<agent_instructions>
  ...
  <deployment>
    User's standard command to run/test the app: `scripts/flutter_build_lock.sh run -d macos`, run from the repo root (`nmtk/neuro_toolkit` also works for the raw `flutter run -d macos` command). The wrapper takes the shared build lock so a dev run cannot race `scripts/build_and_deliver_apk.sh` on the same `build/macos` tree. The dev backend at `moosebun2@192.168.2.90` is already running — this does not deploy or rebuild anything, it just launches the Flutter macOS app locally. `make docker-ex-m` / `scripts/run_dev.sh` are deprecated — do not suggest or use them (see root `AGENTS.md`, "Updating the backend"). Since plain `flutter run` passes no `--dart-define` pointing at the remote host, the app boots against its hardcoded `127.0.0.1` default and lands on `/setup` with no backend found and no saved session, so the sign-in popup ([`server_access_gate.dart`](nmtk/neuro_toolkit/lib/screens/server_access_gate.dart)) appears over the workspace. Use its "Sign in to your server" form ([`server_connect_screen.dart`](nmtk/neuro_toolkit/lib/screens/server_connect_screen.dart)) with the dev host's address plus the app account's username/password — this is an app-level credential only, never SSH, and it works for the non-loopback `192.168.2.90` address directly. SSH/administrator credentials are only ever requested by the separate "set up a new server" flow ([`server_setup_screen.dart`](nmtk/neuro_toolkit/lib/screens/server_setup_screen.dart)), used to provision a brand-new server, not to connect to one that is already running.
  </deployment>
  <end_user_paths>
