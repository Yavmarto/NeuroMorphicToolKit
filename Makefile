...
docker-ex-deploy:
	@if [ -z "$(REMOTE_HOST)" ]; then \
		echo "Error: REMOTE_HOST is not set. Example: make docker-ex-all REMOTE_HOST=user@198.51.100.50"; \
		exit 1; \
	fi
...
deploy-prod: secrets-init
	@if [ -z "$(REMOTE_HOST)" ]; then \
		echo "Error: REMOTE_HOST is not set. Example: make deploy-prod REMOTE_HOST=user@198.51.100.50"; \
		exit 1; \
	fi
...
dev-sync:
	@if [ -z "$(REMOTE_HOST)" ]; then \
		echo "Error: REMOTE_HOST is not set. Example: make dev-sync REMOTE_HOST=user@198.51.100.50"; \
		exit 1; \
	fi
	@REMOTE_HOST=$(REMOTE_HOST) bash scripts/dev_update.sh --skip-tests $(ARGS)

# The default dev backend (AGENTS.md, "Updating the backend").
DEV_BACKEND_HOST ?= moosebun2@192.168.2.90
...
## Defaults to 203.0.113.90 (DEV_BACKEND_HOST). Pass flags with ARGS=, e.g. ARGS='--check-only'.
.PHONY: check-server
check-server:
	@REMOTE_HOST=$(if $(REMOTE_HOST),$(REMOTE_HOST),$(DEV_BACKEND_HOST)) \
...
docker-ex-down:
	@if [ -z "$(REMOTE_HOST)" ]; then \
		echo "Error: REMOTE_HOST is not set. Example: make docker-ex-down REMOTE_HOST=user@198.51.100.50"; \
		exit 1; \
	fi
...
rig-verify:
	@if [ -z "$(RIG)" ]; then echo "Error: RIG is not set. Use 'make rig-verify RIG=dev'"; exit 1; fi
...
rig-provision:
	@if [ -z "$(RIG)" ]; then echo "Error: RIG is not set. Use 'make rig-provision RIG=dev'"; exit 1; fi
