...
After export, runs the six scrub verification commands. All must be empty/zero.
Private-LAN (RFC1918) content is replaced with placeholders before commit; the
single allowed exception is the canonical DEV_BACKEND_HOST default in the
Makefile, so `make dev-update` still works from the public tree.
...
verify_snapshot() {
  ...
  echo "== verification 6: private-LAN (RFC1918) literals (CEL-347) =="
  if ! out=$(python3 "$ROOT/scripts/sanitize_private_lan.py" --check "$dir" 2>&1); then
    echo "$out"
    failed=1
  else
    echo "$out"
...
echo "Replacing private-LAN (RFC1918) content with placeholders (CEL-347) ..."
python3 "$ROOT/scripts/sanitize_private_lan.py" "$OUTPUT_DIR"

cd "$OUTPUT_DIR"
git init -q
git add -A
git commit -q -m "Public snapshot: allowlisted export with private-LAN scrub (CEL-341, CEL-347), no prior history."
git clean -Xqdf
