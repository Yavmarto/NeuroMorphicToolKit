...
run_python_module() {
  ...
  if [ "$mod" = "neurocnl" ]; then
    _stage "pytest-core" "cd neurocnl && python -m pytest neurocnl/ -v --tb=short -p no:nengo --hypothesis-show-statistics"
    ...
    _stage "pytest" "cd '$dir' && $run_cmd pytest --tb=short -p no:nengo --hypothesis-show-statistics"
  fi
