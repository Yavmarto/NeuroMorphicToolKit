...
Usage:
    python3 scripts/cel268_shd_rnn_e2e.py --api-url http://203.0.113.90:9000 --epochs 50
"""
...
def run_e2e(base_url: str, epochs: int, output: Path | None) -> dict[str, Any]:
    ...
    with httpx.Client(headers=_client_headers()) as client:
        ...
        code_hint = (nb.get("diagnostics") or [])
    ...
    eval_events = [e for e in events if e.get("phase") == "eval" or "accuracy" in e]
...
def main() -> int:
    ...
    parser.add_argument(
        "--api-url",
        default="http://203.0.113.90:9000",
