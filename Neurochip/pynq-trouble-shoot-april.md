...
Summary of the multi-day effort to fix `PynqDeployException: Overlay loading failed: No Devices Found` when triggering **Provision Runtime / Deploy** from the Pynq Deployment screen against board `<dev-host>` (Pynq-Z2, `fe37de70-55ce-4357-8a28-ae941b5aba1d`).
...
HTTP 500 from POST http://<dev-host>:8002/hardware/pynq/deploy
...
scp dist/neurochip-0.6.0-py3-none-any.whl xilinx@<dev-host>:/tmp/
```
...
Why: The `overlay_manifest.json.bak` and the hand-written `override.conf` on `<dev-host>` will start to drift from source of truth the moment follow-ups 1–3 are live in provisioning. Keep the board clean.
