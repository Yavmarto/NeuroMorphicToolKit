...
async def test_neurocnl_to_neurosim():
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(f"{NEUROSIM_URL}/api/neurosim/parse-cnl", json={"cnl_spec": spec})
        assert resp.status_code == 200
        graph = resp.json()
        assert "nodes" in graph

        # 3. Run preview in neurosim
        resp = await client.post(f"{NEUROSIM_URL}/api/neurosim/preview", json={
            "graph": graph,
            "duration_ms": 100
        })
        assert resp.status_code == 200
...
async def test_neurosim_to_neurochip():
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(f"{NEUROSIM_URL}/api/neurosim/export/c", json=mock_graph)
        assert resp.status_code == 200
        ...
        resp = await client.post(f"{NEUROCHIP_URL}/api/neurochip/deployments/validate", json=manifest)
        assert resp.status_code == 200
...
async def test_neurosense_to_neurocnl():
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(f"{NEUROSENSE_URL}/api/neurosense/encode", json={
            "data": mock_data,
        ...
        resp = await client.post(f"{NEUROCNL_URL}/api/prosthetic/simulate", json={
            "spec": spec,
            "duration": 0.1,
            "n_neurons": 10
        })
...
async def test_neurohub_orchestration():
    ...
    async with httpx.AsyncClient() as client:
        # 1. Create workflow
        resp = await client.post(f"{NEUROHUB_URL}/api/neurohub/workflows", json=workflow)
        assert resp.status_code == 201

        # 2. Run workflow (requires a project_id)
        # First create a mock project
        project_resp = await client.post(f"{NEUROHUB_URL}/api/neurohub/projects", json={
            "name": "Test Project",
            "description": "Integration Test"
        })
        project_id = project_resp.json()["id"]

        run_resp = await client.post(f"{NEUROHUB_URL}/api/neurohub/workflows/test-integration-wf/run", params={"project_id": project_id})
        assert run_resp.status_code == 200
        run_id = run_resp.json()["id"]

        # 3. Poll for completion
        for _ in range(10):
            status_resp = await client.get(f"{NEUROHUB_URL}/api/neurohub/workflows/runs/{run_id}")
...
async def test_neurobench_benchmarking():
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(f"{NEUROBENCH_URL}/api/neurobench/run", json={
