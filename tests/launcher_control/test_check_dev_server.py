...
def test_help_flag():
    ...
    assert "203.0.113.90" in res.stdout
...
def test_unreachable_host_mocked(tmp_path: Path):
    ...
    assert "Cannot reach dev@203.0.113.90 via SSH" in res.stdout
...
"""
    ...
    env = {**os.environ, "PATH": f"{fake_bin}:{os.environ['PATH']}"}
    res = subprocess.run(
        ["bash", str(SCRIPT), "--check-only", "203.0.113.90"],
        capture_output=True,
        text=True,
        check=False,
        env=env,
