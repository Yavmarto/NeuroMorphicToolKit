...
class TestLauncherHardwareRuntimeProxy(LauncherControlServiceTestBase):
    ...
    def test_pynq_deploy_http_endpoint_propagates_runtime_422(self) -> None:
        ...
        with mock.patch.object(
            ...
            side_effect=launcher_server.RuntimeRequestError(
                (
                    ...
                    "http://203.0.113.99:8002/hardware/pynq/deploy: HTTP 422"
                    f" - {runtime_body}"
                ),
                kind="http",
                url="http://203.0.113.99:8002/hardware/pynq/deploy",
                status_code=422,
        ...
        self.assertNotIn("203.0.113.99", json.dumps(payload))
    ...
    def test_pynq_deploy_http_endpoint_reports_unreachable_runtime_as_bad_gateway(
        ...
        with mock.patch.object(
            ...
            side_effect=launcher_server.RuntimeRequestError(
                (
                    "Runtime request failed for POST "
                    "http://203.0.113.90:8002/hardware/pynq/deploy: "
                    "could not be reached: <urlopen error [Errno 61] Connection refused>"
                ),
                kind="unreachable",
                url="http://203.0.113.90:8002/hardware/pynq/deploy",
            ),
        ):
            request = urllib.request.Request(
                (
        ...
        self.assertNotIn("203.0.113.90:8002", json.dumps(payload))
        ...
        host = self.state.create_akida_host(
            {
                ...
                "username": "operator",
                "password": "secret",
                "runtimeApiUrl": "http://203.0.113.88:8002",
            }
        )
        ...
        host = self.state.create_akida_host(
            {
                ...
                "username": "operator",
                "password": "secret",
                "runtimeApiUrl": "http://203.0.113.89:8002",
            }
        )
