...
def test_list_rigs_default():
    ...
    for rig_id in ("dev", "hp-prodesk", "runner-win-wsl2", "runner-macos"):
        assert rig_id in res.stdout


def test_print_target_resolves_default_rig():
    res = run_script("--print-target", "dev")
    assert res.returncode == 0
    assert res.stdout.strip() == "dev@203.0.113.90"
