...
void main() {
  testWidgets('NmtkSurfaceCard alone in the tree builds without error', (
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: NmtkSurfaceCard(title: 'lonely', child: Text('child')),
        ),
      ),
    );
  ...
  testWidgets('nested NmtkSurfaceCard throws a FlutterError in debug builds', (
    tester,
  ) async {
      ...
      const MaterialApp(
        home: Scaffold(
          body: NmtkSurfaceCard(
            title: 'outer',
            child: NmtkSurfaceCard(title: 'inner', child: SizedBox.shrink()),
          ),
        ),
      ),
    );
