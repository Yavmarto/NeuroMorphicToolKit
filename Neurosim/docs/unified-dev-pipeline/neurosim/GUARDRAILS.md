...
- **Validation Optimization:** Graph validation logic must remain synchronous and highly optimized. Avoid blocking calls to external services during validation.
...
- **Invariant Enforcement:** Never bypass Layer 1 validation when generating CNL from a `CanvasGraph`. The generated spec must always be syntactically and semantically valid.
